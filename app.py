...
def parse_money(s: pd.Series) -> pd.Series:
    ...
    s = s.str.replace(r'^([0-9\.]+)K$', 
                     lambda m: str(float(m.group(1)) * 1e3), regex=True)
    s = s.str.replace(r'^([0-9\.]+)M$', 
                     lambda m: str(float(m.group(1)) * 1e6), regex=True)
    s = s.str.replace(r'^([0-9\.]+)B$', 
                     lambda m: str(float(m.group(1)) * 1e9), regex=True)
...
# Preprocess for ROI-based suggestions
...
df['cast_list']           = df['stars'].fillna('').apply(lambda x: [c.strip() for c in x.strip("[]").replace("'", "").split(',')][:3])
...
def predict():
    ...
    # Predictions
    box_pred    = box_model.predict(df_in[box_features])[0]
    critic_pred = critic_model.predict(df_in[critic_features])[0]
    oscar_pred  = oscar_model.predict(df_in[oscar_features])[0]
...
def recommend():
    data = request.get_json()
    genre = data.get('genre') or data.get('primary_genre')
    if not genre:
        return jsonify({'error': 'Missing genre'}), 400

    candidates = df[df['primary_genre'].str.lower() == genre.lower()]
    if candidates.empty:
        return jsonify([])
